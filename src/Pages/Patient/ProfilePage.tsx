...
import { getDate, getUserName } from "../../utils/getData";
...
const ProfilePage = () => {
  ...
  const [imgErr, setImgErr] = useState<string | boolean>(false);
  ...
  const handleSubmit = async () => {
    if (!edit) {
      ...
      try {
        ...
        if (name!==getName() && user){
         let nameData=getUserName(user.user.role,name)
         if(typeof nameData==="object"){
             formData.append("firstName", nameData.first);  
             formData.append("lastName", nameData.last);  
         }
         else{
             formData.append("firstName", nameData);  
         }
        }
        if (imgFile) {
        ...
        <Grid container>
          ...
          <Grid item xs={3} sx={{ mt: 2, p: 1 }}>
            <TextField
              ...
              disabled={!edit}
              onBlur={() => validName(name, setNameError)}
              onChange={(e) =>
          ...
          <Grid item xs={3} sx={{ mt: 2, p: 1 }}>
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <DatePicker
                ...
                disabled={!edit}
                onChange={(value) => getDate(JSON.stringify(value))}
              />
